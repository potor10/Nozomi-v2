...
class LevelDisplay extends Component {
  ...
  async getLevelCost() {
    ...
    const level_cost_max_res = await fetch(`${process.env.REACT_APP_WEB_URL}/api/` + 
      `collection/characters/level/max_cost/${this.props.server_data.id}/${this.props.character.unit_id}`, fetch_options)
    if (level_cost_res.status === 200 && level_cost_max_res.status ===200) {
      const level_cost = await level_cost_res.json()
      const level_cost_max = await level_cost_max_res.json()
      this.setState({ 
        level_cost: level_cost, 
        level_cost_max: level_cost_max,
        level_cost_loaded: 1
      })
    } else {
  ...
  async levelUp(max) {
    ...
    if (max) {
      mana_cost = this.state.level_cost_max
    } 
  ...
  renderButtons() {
    ...
    let can_buy_max = this.props.user_stats.mana >= this.state.level_cost_max
