...
module.exports = class AccountDatabase {
  ...
  getAll(ids) {
    const stmt_str = "SELECT * FROM accounts WHERE discord_id IN (" + 
      "?, ".repeat(ids.length).slice(0, -2) + ") " +
      " ORDER BY total_power DESC"
    const stmt = this._db.prepare(stmt_str)
    const query = stmt.all(...ids)
    if (query === undefined) throw new ApiException(500, 'ERROR: Not able to find users')
    return query
  }
  ...
  addPower(id, amt) {
    const current_power = this.getUser(id).total_power

    const stmt_str = "UPDATE accounts SET total_power = ? WHERE discord_id = ?"
    const stmt = this._db.prepare(stmt_str)
    const query = stmt.run(current_power + amt, id)

    if (query.changes === 1) {
      return true
    } else {
      throw new ApiException(500, 'ERROR: Total Account Power Was Not Updated')
    }
  }
