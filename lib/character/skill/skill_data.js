...
// Import Constants
import { SKILL_NAMES, EX_SKILL_NAMES, ACTION_PREFIX } from './constants/constants.js'
...
const skillData = (discord_id, server_id, unit_id) => {
  ...
  SKILL_NAMES.forEach(skill_name => {
    ...
    let actions = []
    for (let i = 1; i <= 7; i++) { 
      if (skills_data[skill_id][ACTION_PREFIX+i] !== 0) { 
        actions.push(actions_data[skills_data[skill_id][ACTION_PREFIX+i]])
      }
    }
