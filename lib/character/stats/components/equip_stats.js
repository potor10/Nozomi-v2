...
const equipStats = (unit, master_db) => {
  ...
  for (let i = 1; i <= 6; i++) {
    ...
    if (equipped === 1 && equipment_id !== 999999) {
      equipment_data = master_db.getEquipmentData(equipment_id)
      
      // Equipping an item is an automatic max refine in this bot
      equipment_enhance_data = master_db.getEquipmentEnhance(equipment_id)
      
      STAT_NAMES.forEach(stat => {
        stats[stat] = Math.ceil(equipment_data[equipment_id][stat]) +
          Math.ceil(equipment_enhance_data[equipment_id][stat] * getMaxRefine(equipment_data[equipment_id].promotion_level))
      })
    }
  }
  return stats
