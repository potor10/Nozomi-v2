...
// Import Constants
import { NUMBER_TO_SKILL } from '../constants/constants.js'
...
const skillUpMaxCost = (unit, user, skill_name) => {
  ...
  let skill_unlock_data = master_db.getUnlockSkillData() 

  let unlocked = false
  skill_unlock_data.forEach(unlock => {
  ...
  while (new_level <= unit.level) {
    new_level += 1
    if (mana_cost + skill_cost_data[new_level-1] > user.mana) break
    mana_cost += skill_cost_data[new_level-1]
  }
