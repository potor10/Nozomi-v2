...
module.exports = class CollectionDatabase {
  ...
  addUnit(discord_id, unit_id) {
    ...
    if (query.changes === 1) {
      this.updatePower(discord_id, unit_id)
      return true
    } else {
      throw 'ERROR: Unit Was Not Added'
    }
  }
  ...
  equipCost(discord_id, unit_id, equip_number) {
    ...
    const equip_id = unit_promotion[`equip_slot_${equip_number}`]
  ...
  isEquipped(discord_id, unit_id, equip_number) {
    const stmt_str = `SELECT * FROM collection WHERE discord_id = ? AND unit_id = ?`
    const stmt = this._db.prepare(stmt_str)
    const query = stmt.run(discord_id, unit_id)

    return query[this._NUMBER_TO_EQUIP[equip_number]] === 1
  }

  addEquipment(discord_id, unit_id, equip_number) {
    const equip_cost = this.equipCost(discord_id, unit_id, equip_number)
    if (this.isEquipped(discord_id, unit_id, equip_number)) {
      throw 'Already Equipped!'
    }

    if (!this._account_db.addJewels(discord_id, equip_cost)) {
      throw 'Not Enough Gems!'
    }
    
    const stmt_str = `UPDATE collection SET ${this._NUMBER_TO_EQUIP[equip_number]} = ? WHERE discord_id = ? AND unit_id = ?`
    const stmt = this._db.prepare(stmt_str)
    ...
    if (query.changes === 1) {
      ...
      throw 'ERROR: Equipment Was Not Updated'
    }
  }

  updatePower(discord_id, unit_id) {
    const power = this.calculateStats(discord_id, unit_id).power
    const stmt_str = "UPDATE collection SET total_power = ? WHERE discord_id = ? AND unit_id = ?"
    const stmt = this._db.prepare(stmt_str)
    const query = stmt.run(power, discord_id, unit_id)
    if (query.changes === 1) {
      return true
    } else {
      throw 'ERROR: Power Was Not Updated'
    }
  }
