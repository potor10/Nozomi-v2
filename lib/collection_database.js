...
module.exports = class CollectionDatabase {
  ...
  init() {
    const stmt_str = "CREATE TABLE IF NOT EXISTS collection ( " +
      ...
      `${constants.SKILL_NAMES[0]} INTEGER DEFAULT 1, ${constants.SKILL_NAMES[1]} INTEGER DEFAULT 0, ` +
      `${constants.SKILL_NAMES[2]} INTEGER DEFAULT 0, ${constants.SKILL_NAMES[3]} INTEGER DEFAULT 0, ` + 
      "promotion_level INTEGER DEFAULT 1, " +
      `${constants.NUMBER_TO_EQUIP[1]} INTEGER DEFAULT 0, ${constants.NUMBER_TO_EQUIP[2]} INTEGER DEFAULT 0, ` +
      `${constants.NUMBER_TO_EQUIP[3]} INTEGER DEFAULT 0, ${constants.NUMBER_TO_EQUIP[4]} INTEGER DEFAULT 0, ` +
      `${constants.NUMBER_TO_EQUIP[5]} INTEGER DEFAULT 0, ${constants.NUMBER_TO_EQUIP[6]} INTEGER DEFAULT 0, ` +
  ...
  addUnit(discord_id, unit_id) {
    const unit = this._master_db.getUnitData(unit_id)
    const rarity = this._master_db.getUnitData(unit_id).rarity
    const base_id = parseInt(unit_id.toString().substring(0, 4))

    const stmt_str = "INSERT INTO collection (discord_id, unit_id, base_id, name, rarity) " + 
      "VALUES(?, ?, ?, ?, ?)"
    const stmt = this._db.prepare(stmt_str)
    const query = stmt.run(discord_id, unit_id, base_id, unit.unit_name, rarity)
  ...
  getAllUnits(discord_id, sort_id=0) {
    ...
    const stmt_str = `SELECT * FROM collection WHERE discord_id = ? ORDER BY ${sort_type}`
    const stmt = this._db.prepare(stmt_str)
  ...
  levelUpSkillCost(discord_id, unit_id, skill_idx, max=false) {
    const unit = this.getUnit(discord_id, unit_id)
    const current_skill = constants.NUMBER_TO_SKILL[skill_idx]
  ...
  levelUpSkill(discord_id, unit_id, skill_idx, max=false) {
    ...
    const new_level = (max) ? user.level : unit[constants.NUMBER_TO_SKILL[skill_idx]] + 1
    ...
    const stmt_str = `UPDATE collection SET ${constants.NUMBER_TO_SKILL[skill_idx]} = ? WHERE discord_id = ? AND unit_id = ?`
    const stmt = this._db.prepare(stmt_str)
