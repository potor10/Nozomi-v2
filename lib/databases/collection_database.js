...
import { SKILL_NAMES, EQUIP_SLOT_PREFIX, STAT_NAMES } from '../constants.js'
...
class CollectionDatabase {
   ...
   */
  initDatabase() {
    let stmt_str = "CREATE TABLE IF NOT EXISTS collection ( " +
  ...
  setPromotionLevel(discord_id, unit_id, promotion_level) {
    const stmt_str = "UPDATE collection SET promotion_level = ? WHERE discord_id = ? AND unit_id = ?"
    const stmt = this._db.prepare(stmt_str)
