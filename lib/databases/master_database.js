...
class MasterDatabase {
  ...
  getUnitRarity(unit_id) {
    const stmt_str = "SELECT * FROM unit_rarity WHERE unit_id = ?"
    const stmt = this._db.prepare(stmt_str)
  ...
  getUnitPromotion(unit_id) {
    const stmt_str = "SELECT * FROM unit_promotion WHERE unit_id = ?"
    const stmt = this._db.prepare(stmt_str)
    const query = stmt.all(unit_id)
    return query
  }
  ...
  getEquipmentEnhance(equipment_id) {
    const stmt_str = "SELECT equipment_id, equipment_name, description, " + 
  ...
  /**
   * Obtains the actions of the skill
   * @param {Integer} skill_id the id of skill we are looking for
