...
class MasterDatabase {
  ...
  /**
   * Obtains the banner data from the master database
   * @param {String} time The current time (formatted)
   * @return {Array} array of row objects
   */
  getBanner(time) {
    const stmt_str = "SELECT * FROM banner WHERE start_date <= ? AND end_date >= ?"
    const stmt = this._db.prepare(stmt_str)
    const query = stmt.all(time, time)
    return query
  }
  ...
  /**
   * Obtains data on all the equipment in the game under specified promotion level
   * @param {Integer} promotion_level is an indicator for what promotion level
   * @return {Array} array of row objects containing equipment data
   */
  getEquipmentDataUnderPromotionLevel(promotion_level) {
    const stmt_str = "SELECT * FROM equipment_data WHERE promotion_level < ?"
    const stmt = this._db.prepare(stmt_str)
    const query = stmt.all(promotion_level)
    return query
  }
  ...
  /**
   * Obtains data on all the items in the game
   * @param {Integer} item_type is an optional indicator of which item types to get
   * @return {Array} array of row objects containing item data
   */
  getItemData(item_type) {
    const stmt_str = "SELECT * FROM item_data" + (item_type ? " WHERE item_type = ?" : "")
    const stmt = this._db.prepare(stmt_str)
    const query = item_type ? stmt.all(item_type) : stmt.all()
    return query
  }
