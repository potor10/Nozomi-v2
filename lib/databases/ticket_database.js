...
class TicketDatabase {
  ...
  initDatabase() {
    const stmt_str = "CREATE TABLE IF NOT EXISTS tickets ( " +
      ...
      "ticket_title TEXT, ticket_description TEXT, " + 
   ...
   */
  addTicket(discord_id, server_id, ticket_title, ticket_description, created_on) {
    const stmt_str = "INSERT INTO tickets " + 
      "(discord_id, server_id, ticket_title, ticket_description, created_on) " + 
      "VALUES(?, ?, ?, ?, ?)"
    const stmt = this._db.prepare(stmt_str)
    const query = stmt.run(discord_id, server_id, ticket_title, ticket_description, created_on)
