...
class UserDatabase {
  ...
  initDatabase() {
    const stmt_str = "CREATE TABLE IF NOT EXISTS users ( " +
      ...
      "point_expiration TEXT DEFAULT 0, exchange_points INTEGER DEFAULT 0)"
  ...
  /**
   * Sets the time of exchange point expiration
   * @param {String} discord_id the discord id of the user
   * @param {Integer} point_expiration the next time points will expire
   * @return {boolean} true if successful
   */
  setPointExpiration(discord_id, point_expiration) {
    const stmt_str = "UPDATE users SET point_expiration = ? WHERE discord_id = ?"
    const stmt = this._db.prepare(stmt_str)
    const query = stmt.run(point_expiration, discord_id)
    if (query.changes === 1) return true
    else throw new ApiException(500, 'Point Expiration Time Was Not Updated')
  }
  ...
  setExchangePoints(discord_id, exchange_points) {
    const stmt_str = "UPDATE users SET exchange_points = ? WHERE discord_id = ?"
    const stmt = this._db.prepare(stmt_str)
