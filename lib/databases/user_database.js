...
class UserDatabase {
  ...
  initDatabase() {
    const stmt_str = "CREATE TABLE IF NOT EXISTS users ( " +
      ...
      "daily TEXT DEFAULT 0, total_power INTEGER DEFAULT 0, " + 
      "last_pull TEXT DEFAULT 0, exchange_points INTEGER DEFAULT 0)"
  ...
  setJewels(discord_id, jewels) {
    ...
    if (query.changes === 1) return true
    else throw new ApiException(500, 'Jewels Was Not Updated')
  }
  ...
  setAmulets(discord_id, amulets) {
    ...
    if (query.changes === 1) return true
    else throw new ApiException(500, 'Amulets Was Not Updated')
  }
  ...
  /**
   * Sets the time of last pull of the user
   * @param {String} discord_id the discord id of the user
   * @param {Integer} last_pull the new time of last pull of the user
   * @return {boolean} true if successful
   */
  setLastPull(discord_id, last_pull) {
    const stmt_str = "UPDATE users SET last_pull = ? WHERE discord_id = ?"
    const stmt = this._db.prepare(stmt_str)
    const query = stmt.run(last_pull, discord_id)
    if (query.changes === 1) return true
    else throw new ApiException(500, 'Last Pull Time Was Not Updated')
  }
