...
class UserDatabase {
  ...
  initDatabase() {
    const stmt_str = "CREATE TABLE IF NOT EXISTS users ( " +
      ...
      "last_banner TEXT DEFAULT 0, exchange_points INTEGER DEFAULT 0)"
   ...
   * @param {String} discord_id the discord id of the user
   * @param {Integer} last_banner the new time of last banner of the user
   * @return {boolean} true if successful
   */
  setLastBanner(discord_id, last_banner) {
    const stmt_str = "UPDATE users SET last_banner = ? WHERE discord_id = ?"
    const stmt = this._db.prepare(stmt_str)
    const query = stmt.run(last_banner, discord_id)
    if (query.changes === 1) return true
    else throw new ApiException(500, 'Last Banner Time Was Not Updated')
  }
