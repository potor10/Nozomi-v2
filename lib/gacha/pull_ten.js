...
const pullTen = (discord_id, server_id, gacha_id) => {
  ...
  if (current_gacha.gacha_type === 1) {
    ...
    if (user.jewels >= current_gacha.price * 10) {
      ...
      // add 10 exchange points
      user_db.setExchangePoints(user.discord_id, user.exchange_points + 10)
      user_db.close()
    ...
    user_db.setAmulets(discord_id, user.amulets + amulets_earned)
