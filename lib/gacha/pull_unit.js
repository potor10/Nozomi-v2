...
import gachaExchange from './gacha_exchange.js'

// Import Constants
import { AMULET_CONVERT, RATE_UP } from './constants/gacha_constants.js'
...
const pullUnit = (discord_id, server_id, current_gacha, user, user_db, pity=false) => {
  ...
  if (gacha_type === 2) {
    ...
    // Rate Up Gacha 
    let units_rate_up = gachaExchange(current_gacha.exchange_id)

    for(const unit_id in units_rate_up)  {
      delete gacha_units[unit_id]
    }

    const standard_ids = Object.keys(gacha_units)
    const rate_up_ids = Object.keys(units_rate_up)

    let obtained_unit = {}
    if (Math.random() < RATE_UP) {
      const random_unit_id = rate_up_ids[Math.floor(rate_up_ids.length * Math.random())]
      obtained_unit = rate_up_ids[random_unit_id]
    } else {
      const random_unit_id = standard_ids[Math.floor(standard_ids.length * Math.random())]
      obtained_unit = standard_ids[random_unit_id]
    }

    obtained_unit.dupe = checkExist(discord_id, server_id, obtained_unit.unit_id, user, user_db)
    return obtained_unit
  }
}
